...
func _physics_process(delta: float) -> void:
	...
	if not is_sliding and not is_hit and not is_air_attacking:
		...
		if Input.is_action_just_pressed("dash") and slide_cooldown_timer <= 0.0 \
				and is_on_floor() and not is_crouching:
			_start_slide(direction if direction != 0 else $Pivot.scale.x)

		if Input.is_action_just_pressed("attack") and attack_timer <= 0 \
				and not is_attacking and not is_crouch_attacking:
...
func _update_animation(direction: int) -> void:
	if is_attacking or is_crouch_attacking or is_hit or is_air_attacking:
		return
...
func _crouch_attack() -> void:
	...
	for body in bodies:
		if body.is_in_group("enemies"):
			var knockback_dir = sign(body.global_position.x - global_position.x)
			body.take_damage_enemy(base_damage, knockback_dir)
			print("Crouch ", "CRIT! " if is_crit else "hit! ", "Dealt ", base_damage)
