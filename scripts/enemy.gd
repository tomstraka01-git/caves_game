...
func _physics_process(delta: float) -> void:
	...
	# Locked states — only physics, no AI decisions
	...
	if distance_to_player < DETECT_AREA:
		if distance_to_player > ATTACK_RANGE:
			...
			# Pick heavy or normal — set timer HERE before any async call
			if randi() % HEAVY_ATTACK_CHANCE == 0:
				attack_timer = ATTACK_COOLDOWN + 0.5   # set before async
				state = State.HEAVY_WINDUP
				_heavy_attack_sequence()
			else:
				attack_timer = ATTACK_COOLDOWN          # set before async
				state = State.ATTACK
		else:
			state = State.IDLE
	else:
...
# -------------------------------------------------------
# ATTACK PLAYER (async — timer already set by caller)
# -------------------------------------------------------
...
# -------------------------------------------------------
# HEAVY ATTACK SEQUENCE (telegraph pause then swing)
# -------------------------------------------------------
func _heavy_attack_sequence() -> void:
	...
	velocity.x = 0
	_play_animation("idle")
	await get_tree().create_timer(0.5).timeout  # wind-up window — player can dodge
	if my_id != attack_id or state == State.DEAD:
		state = State.IDLE
		attack_triggered = false
		return
